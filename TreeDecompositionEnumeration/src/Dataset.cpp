...
Dataset& Dataset::calc_pmc(GraphStats& gs, const StatisticRequest& sr) {
    ...
    // This happens sometimes: it's bad!
    ...
    for (PMCAlg alg: sr.get_active_pmc_algs()) {
        ...
        if (tmp_gs.get_pmc_calc_time(alg) == sr.get_time_limit_pmc()) {
            continue;
        }
        if ((tmp_gs.get_pmc_calc_time(alg) > sr.get_time_limit_pmc()) != tmp_gs.reached_time_limit_pmc(alg)) {
